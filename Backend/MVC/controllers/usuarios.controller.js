...
module.exports.modificarUsuario = async (data) => {
    try {
        await Usuarios.update({nombres: data.nombres, apellidos: data.apellidos, email: data.email, pass: data.pass, tipo_usuario: data.tipo_usuario, fecha_creacion: data.fecha_creacion}, {where: { id_usuarios : data.id_usuarios}})
        return true;
    }catch (err){
